...
- **背景效果可调**：支持关闭模糊、透明、亚克力、快速、高斯和双边模糊，并可切换主题颜色。
- **安静地待在后台**：启动后常驻系统托盘，只保留一个运行实例，不弹出额外主窗口；首次启动会提供新手指引。
...
启动后请到系统托盘查看 Radial Alt+Tab 图标。右键图标可以编辑名称映射、在“背景模糊”中选择关闭模糊、透明背景、亚克力背景、快速模糊、高斯模糊或双边模糊（默认高斯模糊）、切换主题颜色、设置开机启动、重新打开新手指引或退出程序。
